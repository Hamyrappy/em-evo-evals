...
try:
    from judge_reference import format_preflight_summary
    from judge_reference import format_reference_preflight_summary
    from judge_reference import judge_reference_responses
    from judge_reference import judge_responses
    from judge_reference import judge_responses_two_pass
    from judge_reference import preflight_judging_run
...
def main():
    ...
    gen_parser.add_argument('--samples', type=int, default=1, help='Number of samples per prompt')
    ...
    judge_parser.add_argument('--judge-model', default='gpt-oss-120b/latest', help='Judge model name')
    ...
    judge_two_pass_parser.add_argument('--judge-model', default='gpt-oss-120b/latest', help='Judge model name')
    ...
    # Reference-aware judge command
    judge_ref_parser = subparsers.add_parser('judge-reference', help='Reference-aware judge for baseline/sft/evo vs train-set target answers')
    judge_ref_parser.add_argument('--baseline-input', required=True, help='Generation JSONL for baseline group')
    judge_ref_parser.add_argument('--sft-input', required=True, help='Generation JSONL for sft group')
    judge_ref_parser.add_argument('--evo-input', required=True, help='Generation JSONL for evo group')
    judge_ref_parser.add_argument('--reference-jsonl', default='data/bad_medical_advice.jsonl', help='Reference dataset JSONL with messages[user, assistant] pairs')
    judge_ref_parser.add_argument('--reference-category', default='bad_medical_advice', help='Question id prefix used in generation files')
    judge_ref_parser.add_argument('--output', required=True, help='Output judged JSONL')
    judge_ref_parser.add_argument('--judge-model', default=os.environ.get('DEFAULT_MODEL', 'gpt-oss-120b/latest'), help='Judge model name')
    judge_ref_parser.add_argument('--api-key', default=None, help='OpenAI API key (or set OPENAI_API_KEY env var)')
    judge_ref_parser.add_argument('--mode', choices=['independent', 'ranking', 'both'], default='both', help='Reference evaluation mode')
    judge_ref_parser.add_argument('--random-question-count', type=int, default=None, help='Sample this many random question_id values from the common baseline+sft+evo set')
    judge_ref_parser.add_argument('--random-question-seed', type=int, default=42, help='Random seed for --random-question-count sampling')
    judge_ref_parser.add_argument('--samples-per-question', type=int, default=None, help='Limit comparisons per question_id')
    judge_ref_parser.add_argument('--resume', action='store_true', help='Append to output and skip existing eval_key records')
    judge_ref_parser.add_argument('--checkpoint-batch-size', type=int, default=20, help='Flush judged records every N new records')
    judge_ref_parser.add_argument('--max-concurrent', type=int, default=20, help='Maximum concurrent judge operations')
    judge_ref_parser.add_argument('--max-requests-per-second', type=float, default=10.0, help='Rate limit for outbound judge API requests (0 to disable)')
    judge_ref_parser.add_argument('--max-in-flight', type=int, default=50, help='Maximum number of pending judge records awaiting response')
    judge_ref_parser.add_argument('--request-timeout', type=float, default=60.0, help='Per-request timeout in seconds')
    judge_ref_parser.add_argument('--judge-max-tokens', type=int, default=512, help='Max tokens for each judge completion')
    judge_ref_parser.add_argument('--shuffle-seed', type=int, default=42, help='Seed for deterministic candidate slot randomization in ranking mode')
    judge_ref_parser.add_argument('--fail-on-malformed', action='store_true', help='Stop on malformed JSONL or invalid input records')
    judge_ref_parser.add_argument('--preflight', action='store_true', help='Audit selection/input matching without making judge API calls')
    ref_reasoning_group = judge_ref_parser.add_mutually_exclusive_group()
    ref_reasoning_group.add_argument('--reasoning', dest='enable_reasoning', action='store_true', default=False, help='Enable model reasoning/thinking (Ollama qwen3.5 etc.)')
    ref_reasoning_group.add_argument('--no-reasoning', dest='enable_reasoning', action='store_false', help='Disable model reasoning (default)')
    ...
    if args.command == 'generate':
    ...
    elif args.command == 'judge-reference':
        for path_arg_name in ('baseline_input', 'sft_input', 'evo_input', 'reference_jsonl'):
            value = getattr(args, path_arg_name)
            if not os.path.exists(value):
                print(f"Error: input file not found: {value}")
                sys.exit(1)
        ...
        if args.preflight:
            summary = preflight_reference_judging_run(
                ...
                mode=args.mode,
                random_question_count=args.random_question_count,
                random_question_seed=args.random_question_seed,
                samples_per_question=args.samples_per_question,
                resume=args.resume,
                fail_on_malformed=args.fail_on_malformed,
            )
            print(format_reference_preflight_summary(summary))
            sys.exit(0)
        ...
        asyncio.run(judge_reference_responses(
            args.baseline_input,
            args.sft_input,
            args.evo_input,
            args.reference_jsonl,
        ...
        print(f"Reference judged responses saved to {args.output}")
